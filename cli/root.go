// Package cli assembles the randomuser command tree from the randomuser
...
// NewApp assembles the kit application from the randomuser domain. The
...
//
// To add a command, declare it in randomuser/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := randomuser.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(randomuser.Domain{}).Register(app)
