...
describes what users get. Pick a format with `-o`, or let randomuser choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
randomuser <command> -o table   # aligned columns for reading
randomuser <command> -o jsonl   # one JSON object per line, for piping
randomuser <command> -o json    # a single JSON array
randomuser <command> -o csv     # spreadsheet friendly
randomuser <command> -o tsv     # tab-separated
randomuser <command> -o url     # just the URL column
randomuser <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
randomuser <command> --fields id,title,url
```
...
```bash
randomuser <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
randomuser <command>            # a table, because this is a terminal
randomuser <command> | wc -l    # JSONL, because this is a pipe
```
